...
import {getDiffieHellman, createDiffieHellman, randomBytes, createCipheriv, createDecipheriv} from "crypto-browserify";
...
export async function createAndComputeDHSecret(){
    ...
    // try{
        const prime = getDiffieHellman('modp15').getPrime();
        const gen = getDiffieHellman('modp15').getGenerator();

        const dh = createDiffieHellman(prime, gen);
        dh.generateKeys();
...
export async function AESEncryption(data, key){
    ...
    // try{
        ...
        const cipher = createCipheriv('aes256', key.subarray(0,32), iv);
...
export function AESDecryption(encryptedImg, iv, key){
    ...
    const cipher = createDecipheriv('aes256', key.subarray(0,32), Buffer.from(iv,'base64'));
